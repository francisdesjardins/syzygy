...
export type StepListCheck<TSteps extends readonly AnyStep[]> = ([MissingNeeds<TSteps>] extends [
  ...
  : {
      readonly [
        K in `antumbra: needs names a step this list does not contain: ${Extract<MissingNeeds<TSteps>, string>}`
      ]: never;
    }) &
  ([DuplicateIds<TSteps>] extends [never]
    ...
    : {
        readonly [
          K in `antumbra: two steps in this list share the id: ${Extract<DuplicateIds<TSteps>, string>}`
        ]: never;
      });
