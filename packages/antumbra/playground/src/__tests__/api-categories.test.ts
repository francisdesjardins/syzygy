...
// By relative path: the unit project runs raw TypeScript with none of the playground's `antumbra`
 ...
 * scoping this to `antumbra` let `ActionRunContext` reach a build. A bare name is not an identity
 * (`HotkeyDef` is documented under `antumbra` *and* `antumbra/react`), so each binding is checked against
 * its own categories; `collectExports` reads named re-exports only, so the wholesale `export *` of
 * the root does not come with it.
 */

const CORE_SPECIFIER = 'antumbra';
...
test.describe('the API reference covers the root', () => {
  ...
  test('every export a binding publishes of its own belongs to a category', () => {
    const orphans = [
      ['react.ts', 'antumbra/react'],
      ['solid.ts', 'antumbra/solid'],
      ['vanilla.ts', 'antumbra/vanilla'],
