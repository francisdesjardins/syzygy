...
`antumbra-playground` — a private Yarn workspace, never published. **Its UI dependencies live here,
...
It consumes the library through the same public specifiers as any user: `antumbra/react` for hooks and
components, `antumbra` for anything that must work without React — see
...
## The microfrontend frame (`public/mfe/` + `mfe-src/` + `vite-plugins/mfe-antumbra.ts`)
...
`antumbra/react`, Support on `antumbra/solid`, Billing on `antumbra/vanilla` over a `<dialog>` hand-written in
...
- `vite-plugins/mfe-antumbra.ts` bundles **all of them in one rolldown build**, so code-splitting
...
1. **Create** `src/pages/<route>/examples/<name>.tsx` — hooks from `antumbra/react`, templates from
