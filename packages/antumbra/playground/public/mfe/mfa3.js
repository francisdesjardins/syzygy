...
//
// The third framework on the page, and the reason it is here: `useDialog` from `antumbra/solid` is
// the same call, with the same options and the same return, as the one Checkout makes from
// `antumbra/react` twenty lines away. Two bindings, one core — which is the claim the library makes
