...
`umbra` → `antumbra`, and the bootstrapper that was `antumbra` is now `umbra`. The two traded names.

`umbra` is the root the other two derive from — _pen_·umbra, _ant_·umbra — so it belongs to the
package the others build on, and that is the bootstrapper: everything boots, and a dialog manager
can perfectly well declare a boot step while the reverse never happens. Meanwhile _ante umbra_, "in
front of the shadow", describes exactly where a modal `<dialog>` renders: the top layer, outside the
normal stacking context. And the antumbra proper is the region past the cone's apex, where the disc
sits wholly inside the source and light gets all the way around it — which is a page still visible
around the thing in front of it, on its backdrop.

The old mapping was defensible: `showModal()` makes the rest of the document `inert`, and total
occlusion is what an umbra is. This is not a correction of an error, it is a choice between two
readings — taken now because the names freeze once a third library joins, and because nothing here
is published, so there is no ecosystem to break.

**What moved:** the directory, the package name, every import specifier, the playground's name, the
mascot component, the Vite plugin and the micro-frontend entry files. **What did not:** the mascot's
drawing and its smirk, which belong to what this library does rather than to what it is called. Its
favicon did swap, because that one draws the shadow region itself — a wide disc with a corona around
it is an umbra, and this is no longer one. The two playground tabs trade icons.

Nothing in the deployed URLs changed. They are named for the capability — `/playground/dialog` — for
