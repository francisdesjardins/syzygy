...
/** `antumbra/react`'s hook of the same name; {@link useDialogManager} is the live-snapshot one. */
export function useDialogManagerContext(): DialogManager {
  return useContext(DialogManagerContext);
}

/**
 * Provides an isolated `DialogManager` to descendant hooks, as `antumbra/react`'s does and for the
