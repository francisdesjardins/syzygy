 ...
 * naming an export that is gone. The pattern below only matches `antumbra` specifiers, so the file's
...
const IMPORT_PATTERN =
  /import(\s+type)?\s*\{([^}]*)\}\s*from\s*'(antumbra(?:\/(?:react|solid|vanilla))?)';/g;
...
test.describe('documented imports', () => {
  test('every value symbol the docs import is exported by the stated entry point', () => {
    ...
    const exportsBySpecifier: Record<string, readonly string[]> = {
      antumbra: rootExports,
      'antumbra/react': reactExports,
      'antumbra/solid': solidExports,
      'antumbra/vanilla': vanillaExports,
    };
