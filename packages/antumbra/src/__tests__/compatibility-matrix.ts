...
export function bindingCells(row: BindingRow): readonly (readonly [string, Cell])[] {
  return [
    ['antumbra/react', row.react],
    ['antumbra/solid', row.solid],
    ['antumbra/vanilla', row.vanilla],
  ];
}
...
export const OPTION_ROWS: readonly OptionRow[] = [
  ...
  },
  {
    ...
    enforcement: 'RUNTIME',
    note: '`syncLabellingDiagnostics` reports ids that resolve to nothing — reading the element, not the options, because in `antumbra/vanilla` the markup is the caller’s. Silent until `setLogLevel`.',
    references: [
      {
        file: 'src/vanilla/__tests__/bind-dialog.ct.tsx',
...
export const BINDING_ROWS: readonly BindingRow[] = [
  ...
  },
  {
    ...
    solid: { state: 'n-a', note: 'Same — `antumbra/solid` builds its own `<dialog>`.' },
    vanilla: {
      state: 'works',
...
export const PLATFORM_ROWS: readonly PlatformRow[] = [
  ...
  },
  {
    ...
    state: 'no-platform',
    why: '`close()` queues the event, so it arrives with `dialog.open` already back to `true` — which is the only guard a listener has for telling a raise from a real close. It matters most in `antumbra/vanilla`, where the listener is the caller’s.',
    references: [
      {
        file: 'src/vanilla/__tests__/bind-dialog.ct.tsx',
...
export function renderMatrix(): string {
  ...
  lines.push('');
  lines.push('| Capability | `antumbra/react` | `antumbra/solid` | `antumbra/vanilla` |');
  lines.push('| --- | --- | --- | --- |');
