# antumbra — reprise dans un dépôt neuf
...
Un dépôt neuf, à la maison, sous le nom `antumbra` (libre sur npm). Rien à migrer : `git init`,
copier le contenu de cette archive, premier commit.

À terme : un **monorepo** avec `umbra` (gestionnaire de dialogues), `penumbra` (le design system) et
`antumbra`, avec un **playground partagé**. C'est pour ça que le playground d'ici copie la structure
de celui d'umbra jusqu'aux noms de fichiers — le jour où les trois se rejoignent, il n'y a pas deux
...
- Le monorepo à trois paquets avec le playground partagé, quand umbra et penumbra suivront.
