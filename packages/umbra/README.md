...
**[Open the playground →](https://francisdesjardins.ca/playground/boot/#/getting-started)**
...
umbra takes that work, derives the parallelism from the dependencies you declare, and hands back
...
```tsx
import { BootstrapProvider, useBootstrapContext, useStepData, useIntentHost } from 'umbra/react';
...
Four entry points. `umbra` is the core and resolves with no framework installed. `umbra/react`
and `umbra/solid` are the hook bindings, each reaching only its own framework.
`umbra/plain` is a controller that connects the intent queue to markup you already wrote, with
no framework at all.
...
then the CHANGELOG records every rename, organised by date, and it explains _why_ each name moved
