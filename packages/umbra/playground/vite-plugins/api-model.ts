...
export function typedocFailure(error: unknown): Error {
  ...
  return new Error(
    output === ''
      ? `[umbra-api] typedoc failed: ${reason}`
      : `[umbra-api] typedoc failed — its own output follows.
...
async function buildModel(
    ...
    return {
      ...
      symbols: category.symbols.map((name) => {
        ...
        if (declaration === undefined) {
          throw new Error(
            `[umbra-api] category "${category.id}" lists "${name}", which "${category.specifier}" does not export.`
          );
        }
        ...
        if (symbol === null) {
          throw new Error(
            `[umbra-api] "${symbolKey(category.specifier, name)}" has no renderable kind.`
          );
        }
        return symbol;
      }),
  ...
  if (orphans.length > 0) {
    throw new Error(
      `[umbra-api] ${String(orphans.length)} export(s) belong to no category and would be ` +
...
export function apiModelPlugin(): Plugin {
  ...
  const cacheDir = join(repoRoot, 'node_modules', '.cache', 'umbra-api');
