/**
 * umbra's mascot: antumbra's drawing, to the stroke — the same inverted heraldic sun, the same
 * eight-flame corona, the same disc. **Only the face differs.** They are the same eclipse seen from
 * a different distance, so they are the same drawing wearing a different expression.
 *
 * Where antumbra smirks — it spends its life putting a shadow over your page and waiting for you to
 * deal with it — this one watches. The umbra is the total shadow, the part of the cone where the
 * source is gone entirely: the dark this library runs in, before anything has been lit. Its job is
 * to start everything while there is still nothing to see, and then wait on an answer it cannot
 * hurry. Eight flames drawn from inside the disc
  ...
  return (
    ...
    >
      ...
      <g fill="none" stroke={ink} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round">
        {/* Brows lifted and level, where antumbra's angle down into the smirk. Still a clear gap over
