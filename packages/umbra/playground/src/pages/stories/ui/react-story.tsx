import { BootstrapProvider, useStepData, useBootstrapContext, useIntentHost } from 'umbra/react';
import type { HostCapabilities } from 'umbra/react';
...
/**
 * One value per assertion, addressed by `data-testid` and nothing else.
 *
 * `READOUT` is the contract with the spec — the same ids the Solid harness publishes, so one test
 * file answers for both bindings.
 */
function Readout() {
...
export function mountReactStory(host: HTMLElement): () => void {
  ...
  return () => {
    ...
    // cleanup can be exactly that. A microtask puts the teardown after the commit it would
    // otherwise interrupt.
    queueMicrotask(() => {
      root.unmount();
      container.remove();
