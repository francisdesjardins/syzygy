 ...
 * host down and dispose the live run between an intent being forwarded and the user answering it —
 ...
 * effect depends on the live run and the port, and both are honestly reactive.
 *
...
export function useIntentHost(host: HostCapabilities): readonly ForwardedIntent[] {
  const { live } = useBootstrapContext();
  ...
  useEffect(() => {
    ...
    const attached = attachIntentHost(live, {
      host,
