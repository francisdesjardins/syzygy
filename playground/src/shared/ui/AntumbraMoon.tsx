/**
 * antumbra's mascot: umbra's drawing, to the stroke — the same inverted heraldic sun, the same
 * eight-flame corona, the same disc. **Only the face differs**, and that is the whole point while
 * the two libraries are siblings rather than one monorepo: they are the same eclipse seen from a
 * different distance, so they are the same drawing wearing a different expression.
 *
 * Where umbra smirks — it spends its life putting a shadow over your page and waiting for you to
 * deal with it — this one watches. An antumbra is the ring past the apex, where the light gets all
 * the way around, and the library's job is to start everything and then wait on an answer it
 * cannot hurry. Eight flames drawn from inside the disc
...
export function AntumbraMoon({
  ...
  return (
    <svg
      ...
      viewBox="0 0 200 200"
      role="img"
      aria-label="An annular eclipse with a face, peeking"
    ...
    >
      ...
      {/* The face, engraved in light — strokes only, a woodcut has no fills. Attentive rather than
          smug: this one is watching a run it started and cannot hurry along. */}
      <g fill="none" stroke={ink} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round">
        {/* Brows lifted and level, where umbra's angle down into the smirk. Still a clear gap over
            the nose — let them meet and the face gains a unibrow. */}
        <path d="M64 76 C72 64, 85 62, 94 70" strokeWidth="6.5" />
        <path d="M106 70 C115 62, 128 64, 136 76" strokeWidth="6.5" />

        <g className="um-eyes">
          {/* Round and open, not narrowed by a straight lid: an antumbra is the ring where the
